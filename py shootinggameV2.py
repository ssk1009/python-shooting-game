...
class LaserBoss(Enemy):
    ...
    def update(self, player_rect, platforms, dt):
        ...
        if self.state == "MOVE":
            ...
            if self.timer > 2000: self.state = "MOVE"; self.timer = 0; self.is_firing = False
    ...
    def process_collisions(self):
        ...
        for item in self.items[:]:
            if self.player.rect.colliderect(item.rect):
                if item.type == "double_shot": self.player.has_double_shot = True; self.player.item_timer = 8000
                elif item.type == "heal": self.player.hp = min(self.player.max_hp, self.player.hp + 30) 
        ...
        for m in self.missiles[:]:
            for e in self.enemies[:]:
                if m.rect.colliderect(e.rect):
                    e.hp -= 40; self.missiles.remove(m) if m in self.missiles else None
                    if e.hp <= 0: self.enemies.remove(e); self.player.score += 100
            
            for b in self.bosses[:]:
                if m.rect.colliderect(b.rect):
                    b.hp -= 20; self.missiles.remove(m) if m in self.missiles else None
                    if b.hp <= 0: 
                        self.player.score += 2000; self.trigger_shake(500, 10)
            ...
            if isinstance(b, LaserBoss) and b.is_firing:
                if self.player.rect.colliderect(b.laser_rect):
                    if self.player.shield_timer <= 0 and self.player.invincible_timer <= 0:
                        self.player.hp -= 2; self.player.invincible_timer = 100; self.trigger_shake(150, 5)
