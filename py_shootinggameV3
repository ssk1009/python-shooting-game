...
class Entity(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, color):
        super().__init__() # 반드시 Sprite 초기화 호출
        self.true_x = float(x)
        self.true_y = float(y)
...
class Player(Entity):
    ...
    def shoot(self, missile_group):
        offsets = [-DOUBLE_SHOT_OFFSET, DOUBLE_SHOT_OFFSET] if self.has_double_shot else [0]
        for off in offsets:
    ...
    def update(self, platforms, dt):
        ...
        if self.has_double_shot:
            self.item_timer -= dt
            if self.item_timer <= 0: self.has_double_shot = False
...
class Enemy(Entity):
    ...
    def update(self, player_rect, platforms, dt, *args): # *args 처리로 다형성 확보
        dt_sec = dt / 1000.0
...
class RangedEnemy(Enemy):
    ...
    def update(self, player_rect, platforms, dt, enemy_bullets_group=None):
        ...
        if abs(dist) > RANGED_APPROACH_DIST: self.true_x += self.speed * dt_sec if dist > 0 else -self.speed * dt_sec
        elif abs(dist) < RANGED_FLEE_DIST: self.true_x -= self.speed * dt_sec if dist > 0 else -self.speed * dt_sec
...
class DashBoss(Enemy):
    def __init__(self, x, y):
        ...
        self.hp = DBOSS_MAX_HP; self.max_hp = DBOSS_MAX_HP
        self.state = "IDLE"
        self.timer = 0
        self.dash_dir = 1
    ...
    def update(self, player_rect, platforms, dt, *args):
        ...
        if self.state in ["DASH", "JUMP"]:
            if self.rect.left <= 0:
                self.rect.left = 0; self.sync_from_rect(); self.dash_dir = 1
            elif self.rect.right >= SCREEN_WIDTH:
                self.rect.right = SCREEN_WIDTH; self.sync_from_rect(); self.dash_dir = -1
...
class LaserBoss(Enemy):
    def __init__(self, x, y):
        ...
        self.hp = LBOSS_MAX_HP; self.max_hp = LBOSS_MAX_HP
        self.color = (100, 0, 100)
        self.state = "MOVE"; self.timer = 0
        self.laser_rect = pygame.Rect(0, 0, 0, 0); self.is_firing = False
    ...
    def update(self, player_rect, platforms, dt, *args):
        ...
        if self.state == "MOVE":
            ...
            self.update_rect()
            if self.timer > LBOSS_MOVE_TIME: self.state = "CHARGE"; self.timer = 0
        elif self.state == "CHARGE":
            self.color = (200, 100, 255)
            if self.timer > LBOSS_CHARGE_TIME:
                self.state = "FIRE"; self.timer = 0; self.is_firing = True
            ...
            if self.timer > LBOSS_FIRE_TIME: self.state = "MOVE"; self.timer = 0; self.is_firing = False
...
class Item(Entity):
    def __init__(self, x, y, item_type="double_shot"):
        color_map = {"double_shot": MAGENTA, "heal": GREEN, "shield": CYAN}
...
class Game:
    def __init__(self):
        self.platforms = [
            ...
            pygame.Rect(200, 450, 200, 20), pygame.Rect(450, 300, 200, 20), pygame.Rect(150, 180, 200, 20)
        ]
        self.shake_timer = 0; self.shake_intensity = 0
        self.reset()
    ...
    def reset(self):
        ...
        # 구조적 개선: Pygame 내장 Group을 통한 자동화된 객체 관리
        ...
        self.bosses.add(DashBoss(600, 400) if random.random() > 0.5 else LaserBoss(400, 150))
        self.items.add(Item(400, 300))
    ...
    def process_collisions(self):
        if self.player.rect.top > SCREEN_HEIGHT:
            self.player.hp = 0; self.ui_message = "낭떠러지로 추락!"; self.ui_message_timer = 3000

        # 개선 1: 아이템 충돌 (충돌한 아이템은 즉시 그룹에서 제거됨)
        hit_items = pygame.sprite.spritecollide(self.player, self.items, True)
        for item in hit_items:
            if item.type == "double_shot": self.player.has_double_shot = True; self.player.item_timer = ITEM_DOUBLE_SHOT_DUR
            elif item.type == "heal": self.player.hp = min(self.player.max_hp, self.player.hp + ITEM_HEAL_AMOUNT)
            elif item.type == "shield": self.player.shield_timer = ITEM_SHIELD_DUR
            self.player.score += SCORE_ITEM_PICKUP

        # 개선 2: 미사일 vs 적 (미사일은 삭제 처리됨)
        ...
        for enemy, hit_missiles in enemy_hits.items():
            ...
            if enemy.hp <= 0:
                enemy.kill() # O(1) 시간에 모든 그룹 참조 안전하게 해제
    ...
    def update(self, dt):
        ...
        for b in self.bosses:
            ...
            if isinstance(b, LaserBoss) and b.is_firing: self.trigger_shake(*SHAKE_LASER_FIRING)
        ...
        if self.item_spawn_timer >= ITEM_SPAWN_INTERVAL:
            ...
            if len(self.items) < 3:
                ...
                while not valid_position:
                    spawn_x = random.randint(50, SCREEN_WIDTH - 50); spawn_y = random.randint(100, SCREEN_HEIGHT - 100)
                    temp_rect = pygame.Rect(spawn_x, spawn_y, 25, 25); collision = any(temp_rect.colliderect(p) for p in self.platforms)
                    if not collision: valid_position = True
                self.items.add(Item(spawn_x, spawn_y, random.choice(["double_shot", "heal", "shield"])))
        ...
        if self.enemy_spawn_timer >= spawn_delay:
            self.enemy_spawn_timer = 0
            new_enemy = RangedEnemy(random.choice([0, 750]), 0, self.level) if random.random() > 0.5 else Enemy(random.choice([0, 750]), 0, self.level)
            self.enemies.add(new_enemy)

        # 각 엔티티의 화면 이탈 판정은 update 내부에서 self.kill()로 자율 처리됨
        for e in self.enemies: e.update(self.player.rect, self.platforms, dt, self.enemy_bullets)
    ...
    def draw(self, screen):
        temp_surface = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)); temp_surface.fill(BG_COLOR)
        for plat in self.platforms: pygame.draw.rect(temp_surface, GREEN, plat)
        
        # Group을 순회하며 custom draw를 호출
        ...
        for b in self.bosses: b.draw(temp_surface)
        
        self.player.draw(temp_surface); self.draw_ui(temp_surface)
        ...
        screen.fill((0, 0, 0)); screen.blit(temp_surface, (shake_x, shake_y))
    ...
    def draw_ui(self, surface):
        ...
        if self.game_over:
            msg = large_font.render("GAME OVER (R)", True, WHITE); surface.blit(msg, (SCREEN_WIDTH//2-150, SCREEN_HEIGHT//2-50))
        if self.ui_message_timer > 0:
            msg_txt = font.render(self.ui_message, True, WHITE); msg_rect = msg_txt.get_rect(center=(SCREEN_WIDTH // 2, 80))
            pygame.draw.rect(surface, (0, 0, 0), msg_rect.inflate(30, 15)); surface.blit(msg_txt, msg_rect)
